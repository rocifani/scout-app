...
// Crear nuevo valor de cuota / afiliación (usa tu RPC)
...
export async function marcarCuotaPagadaAction(formData: FormData) {
  ...
  if (!Number.isFinite(cuotaId) || cuotaId <= 0) throw new Error("cuota_id inválido.");

  const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD

  const { error } = await supabaseAdmin.from("cuotas").update({ fecha_pago: today }).eq("id", cuotaId);
  if (error) throw new Error(error.message);

  revalidatePath("/admin/cuotas");
}

// Marcar cuota como impaga (borra fecha)
export async function marcarCuotaImpagaAction(formData: FormData) {
  ...
  const { error } = await supabaseAdmin.from("cuotas").update({ fecha_pago: null }).eq("id", cuotaId);
  if (error) throw new Error(error.message);
